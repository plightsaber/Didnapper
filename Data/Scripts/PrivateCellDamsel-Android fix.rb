...
class PrivateCellDamsel
  ...
  include FaceLib
    ...
    @image_list = {}
    @last_pass = {}
    @msg_view = Viewport.new(0, 0, 640, 480)
    @msg_view.z = 500
    @message_img = Sprite.new(@msg_view)
  ...
  def generateMisc (mood,tickle)
    ...
    #Ghirah - modified loop body to essentially detect changes
    first_pass = (@image_list == {}) ? true : false
    @last_pass = @image_list
    @image_list = {}
		commandArray.each do |command|
      # Show picture - attempt1
      #$game_screen.pictures[command[0]].show(command[1], 0,
      #  command[4], command[5], 100, 100, 255, 0)
      
      if !@viewports.has_key?(command[0]) #create viewport for each z-level encountered
        @viewports[command[0]] = Viewport.new(0, 0, 640, 480)
        @viewports[command[0]].z = 450+command[0]
      end
      name = command[1]
      if @image_list.has_key?(name)
        #do nothing
      elsif !@last_pass.has_key?(name) #if not drawn, draw
        bit = RPG::Cache.picture(name)
        pic = Sprite.new(@viewports[command[0]])
        pic.bitmap = bit
        pic.x, pic.y, pic.visible = command[4], command[5], false
        @image_list[name] = pic
      else
        @image_list[name] = @last_pass[name]
        @last_pass.reject! {|key, value| key == name}
        #@last_pass.except!(name)
      end
      #old
			#pic = RPG::Cache.picture(command[1])
			#$override_pic['pc_canvas'].blt(command[4],command[5],pic,Rect.new(0,0,pic.width,pic.height))
		end
    #Ghirah - remove outdated pictures
    @image_list.each do |key, value|
      value.visible = !first_pass
    end
    @last_pass.each do |key, value|
      value.dispose
    end
    @last_pass = {}
    
    #$override_pic['pc_canvas'] = RPG::Cache.picture('PCEstherBase')
    #if @message_img.disposed?
    #  @message_img = Sprite.new(@msg_view)
    #end
    @message_img.visible = true
    message = getMessage
    if message['message'] != ''
      ...
      @message_img.bitmap = bitm
      @message_img.opacity = opacity
      #$override_pic['pc_canvas'].blt(0,0,bitm,bitm.rect,opacity)
   end
		#still need this to display pc_canvas
    #Ghirah: commented this out
		#$data_common_events[96].list = [RPG::EventCommand.new(231,231,[30,'pc_canvas',0,0,0,0,100,100,255,0]),endEvent]
  end
  
  def dispose
    @message_img.visible = false #just doing this to preserve a small quirk of how original PC works
    #@message_img.dispose #space of one image shouldn't matter much, esp this one
    @image_list.each do |key, image|
      image.dispose
    end
    @last_pass.each do |key, image|
      if !image.disposed?
        image.dispose
      end
    end
    @viewports.each do |key, vp|
      vp.dispose
    end
    @image_list, @last_pass, @viewports = {}, {}, {}
  end
end
